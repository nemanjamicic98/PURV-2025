...
static long etx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int32_t nova_vrijednost;
         ...
         switch(cmd) {
                case WR_VALUE:
                        if( copy_from_user(cells ,(int32_t*) arg, sizeof(cells)) )
                        {
                ...
                case RD_VALUE:
                        if(!data_written)  
                        {
							//Zahtjev za citanje,ali nije upisano nista.
						...
						if(copy_to_user((int32_t*) arg, &nova_vrijednost, sizeof(nova_vrijednost)))
						{
