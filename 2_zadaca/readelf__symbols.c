...
char ELF_SYMBOL_VISIBILITY[7][15] = {"DEFAULT", "INTERNAL", "HIDDEN", "PROTECTED","EXPORTED", "SINGLETON", "ELIMINATE"};

char ELF_INFO_TYPE[12][16] = {"NOTYPE", "OBJECT", "FUNC", "SECTION", "FILE", "COMMON", "TLS", "LOOS", "HIOS", "LOPROC", "SPARC_REGISTER", "HIPROC"};

char ELF_INFO_BIND[7][10] = {"LOCAL", "GLOBAL", "WEAK", "LOOS", "HIOS", "LOPROC", "HIPROC"};

//Pronalazi simboličko ime za indeks sekcije simbola (st_shndx)
//ako vrijednost nije prepoznata vraca broj sekcije kao string
...
void print_symbols(Elf64_Sym *symbol_table, int num_of_entries, char *names){
  ...
  for(int i = 0; i < num_of_entries; i++){
    ...
    section_indexes_mapping(&section_index, symbol_table[i].st_shndx); 	//st_shndx-sadrzi indeks sekcije u kojoj se simbol nalazi
    printf("%-9s", section_index);
    printf(" %-30s", names + symbol_table[i].st_name);					//symbol_table[i].st_name – offset unutar te string tabele gde se nalazi ime simbola
    printf("\n");
...
int main(int argc, char* argv[]){
  ...
  /* za ekstrakciju podatka iz fajla koriste se strukture definisane u elf.h zaglavlju */
...
//fstat(fd, &sb) popunjava strukturu sb podacima o fajlu, uključujući njegovu veličinu (sb.st_size).
//ovo je bitno jer nam treba veličina fajla da bismo ga kasnije mapirali u memoriju (mmap)
  struct stat sb;
  if(fstat(fd, &sb) == -1){
  ...
  MAP_PRIVATE-promijene u memoriji nece uticati na fjl na disku*/
  ...
  /* dio koda koji prolazi kroz sve sekcije fajla i na osnovu polja koje daje tip (sh_type) pronalazi tabele simbola .symtab i .dynsym */
