...
static void push_and_print_stats(int dd) {
	...
	{
        printf("Stats: gen=%llu live=%llu born=%llu died=%llu\n",(unsigned long long)st.generations,(unsigned long long)st.total_live,(unsigned long long)st.born,(unsigned long long)st.died);
    } 
	else 
	{
...
int main(int argc, char* argv[])
{
    ...
    int _tmp = system("clear"); /*nije moglo system("clear")*/
	(void)_tmp;
    ...
    while (!stop_requested)
    {
		...
		/*vrijeme izmedju generacije*/
        if (vrijeme_pauze > 0)
			sleep((unsigned)vrijeme_pauze); 
    }

    /* zaustavi niti i očisti resurse */
    stop_requested = 1;
    pthread_mutex_lock(&turn_mx);
    pthread_cond_broadcast(&turn_cv);
    pthread_mutex_unlock(&turn_mx);
